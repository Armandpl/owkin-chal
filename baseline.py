...
if __name__ == "__main__":
    ...
    filenames_train = [os.path.join(train_dir, "ID_{}.npy".format(str(idx).zfill(3))) for idx in train_output["ID"]]
    ...
    # filenames_test = sorted(glob(test_dir+"*.npy"))
    filenames_test = [os.path.join(test_dir, fname) for fname in os.listdir(test_dir)]
    # for filename in filenames_test:
    #    assert filename.is_file(), filename
    ids_test = [f.replace(".npy","") for f in filenames_test]
